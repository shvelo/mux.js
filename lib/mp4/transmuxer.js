...
AudioSegmentStream = function(track, options) {
  ...
  this.flush = function() {
    ...
    moof = mp4.moof(sequenceNumber, [track], options);
...
Transmuxer = function(options) {
  ...
  if(typeof options.pssh === 'string') {
    options.pssh = conversionUtils.base64ToUint8(options.pssh);
  }
  if(typeof options.kid === 'string') {
    options.kid = conversionUtils.hexStringToUint8(options.kid.replace('-',''));
  }
  if(typeof options.iv === 'string') {
    options.kid = conversionUtils.hexStringToUint8(options.iv.replace('-',''));
  }
  if(options.iv instanceof Uint8Array) {
    options.iv_size = options.iv.byteLength;
  }
